...
# function to get the amount of charge required to complete a specified route  
def get_charge_required(distance_traveled, durationMinutes, time_of_year, bus_type, eval_type):
    ...
    See file readme for different model parameter settings. 
...
def charge_status_via_trip_completion(trips_flattened_df, blockID, start_charge_pct,
    ...
    bus_type: str, choose 'eastLib' or 'brt' or 'allRoutes'
        Dictates whether to adjust the charge depletion rate if it is 40 ft vs 60 ft
    min_charge_time: int
...
def failed_block_loop(df, blockID, start_charge_pct, min_charge_threshold, 
    ...
    min_charge_time: int
        minimum number of minutes required for charge to happen. Usually flags a layover period
    numberOchargers : object to track charger count/placement
    
    Purpose
    -------
    Assesses charging options (project amount of charge, charger placement, charging options)
            based on failed blocks

    Returns
    -------
    list
        chargeNeeded, charge_options, ChargePoints, numberOchargers
